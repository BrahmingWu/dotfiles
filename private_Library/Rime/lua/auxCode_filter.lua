...
function AuxFilter.init(env)
	...
	-- 设定是否显示辅助码，默认为显示
	env.show_aux_notice = config:get_string("axu_code/show_aux_notice") or "trigger"
		...
		for cand in input:iter() do
			...
			if env.show_aux_notice and auxCodes and #auxCodes > 0 then
				...
				elseif env.show_aux_notice == "trigger" then
					local localSplit = inputCode:match(env.trigger_key_string .. "([^,]+)")
					if localSplit and #localSplit > 0 then
			...
			elseif
				#auxStr > 0
				and fullAuxCodes
				and (cand.type == "user_phrase" or cand.type == "phrase")
				and AuxFilter.match(fullAuxCodes, auxStr)
			then
